...
#include <fstream>
// Math - STD math Library
//#include <bits/stdc++.h>
#include <math.h>
// Stdio - Standart input output library
#include <stdio.h>
// algorithm - STD template Library
#include <algorithm>
//#include  <utility>
 const float RESOLUTION = 0.0000001f;
...
// Structure: Point
	//
	// Description: A 2D point implementation with integer x, y references
	struct Point2i 	{
		int x;
		int y;

		Point2i () : x(0), y(0) {}
		Point2i (int inX, int inY) : x(inX), y(inY) {}
		inline friend Point2i operator + (const Point2i& a, const Point2i& b);
		inline friend Point2i operator - (const Point2i& a, const Point2i& b);
	};
	...
	// Structure: Rect
	//
	// Description: A rect implementation with left,top,right,bottom integer references
	struct Rect
	{
		int left;
		int top;
...
//		Vector3 (int n)	: X(n), Y(n), Z(n)   { }
	...
	// Return Vector length
		...
		// Single argument Addition Operator Overload
		...
		Vector3 cross(const Vector3 a, const Vector3 other)	{
	...
	// Structure: 4D Matrix
	//
	// Description: 4d Matrix using left, up, forward and position notation
	//////////////////////////////////////////////////////////////////////////////
	struct Matrix4
	{
	    float m[16];
	...
	private:
	    ...
	    static void SwapRows (Matrix4& m, int r1, int r2);
	    static void DivideRow (Matrix4& m, int r, float f);
	    static void SubMultRow (Matrix4& m, int dest, int src, float f);
	};
	...
	Vector3	Matrix4::Right () const	{
	    return Vector3( At(0, 0), At(0, 1), At(0, 2) );
	}

	Vector3	Matrix4::Up () const	{
	    return Vector3( At(1, 0), At(1, 1), At(1, 2) );
	}

	Vector3	Matrix4::Forward () const
	{
	    return Vector3( At(2, 0), At(2, 1), At(2, 2) );
	}



	void Matrix4::SwapRows (Matrix4& m, int r1, int r2)
	{
	    for (int c = 0; c < 4; ++c) {
	    	int temp = m.At(r2, c);
	    	m.At(r2, c) = m.At(r1, c);
	...
	void
	Matrix4::DivideRow (Matrix4& m, int r, float f)
	{
	    for (int c = 0; c < 4; ++c) {
	        m.At(r, c) /= f;
	    }
	...
	void
	Matrix4::SubMultRow (Matrix4& m, int dest, int src, float f)
	{
	    for (int c = 0; c < 4; ++c) {
